...
export class UwsPlatformAdapter extends AbstractHttpAdapter {
   ...
   *
   * If a callback is provided and listening fails, the error is passed to the callback.
   * If no callback is provided, the error is thrown asynchronously (crashes the process).
   */
  ...
  listen(port: number, ...args: any[]): void {
    ...
    this.uwsApp.listen(hostname, port, (socket) => {
      if (socket) {
        ...
        if (callback) {
          // Pass error to callback (Node.js convention: callback(error))
   ...
   * @returns The unmodified handler
   */
  applyVersionFilter(
    handler: Function,
    _version: any,
