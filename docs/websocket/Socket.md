...
handleSecure(@ConnectedSocket() client: UwsSocket) {
  ...
  // Access user data with optional chaining
...
export class StreamGateway {
  ...
  async handleLargeFile(@ConnectedSocket() client: UwsSocket) {
    try {
      ...
      console.error(`File transfer failed for client ${client.id}:`, error);
      client.emit('file-error', { message: 'Transfer failed due to slow connection' });
      client.disconnect();
    }
  }
  ...
  private async waitForDrain(
    ...
    return new Promise((resolve, reject) => {
      ...
      // This prevents resource leaks when client connection is frozen, slow, or disconnected
      const timeout = setTimeout(() => {
        clearInterval(check);
        reject(new Error('Drain timeout exceeded - client may be disconnected or too slow'));
      }, 30000);
