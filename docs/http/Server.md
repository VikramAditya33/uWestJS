...
- **10x faster** than Express for HTTP requests
...
Trust proxy headers (X-Forwarded-*). Can be:
...
- `callback` - Optional callback when server starts
...
interface StaticFileOptions {
  ...
  etag?: boolean;            // Enable ETags (default: true)
