...
handler(@Req() req: UwsRequest) {
  ...
  console.log(req.query.tags);  // ['a', 'b']
}
```
...
Parsed request body. Populated by body parser middleware based on Content-Type.
...
async handleWebhook(@Req() req: UwsRequest) {
  ...
  const rawBody = req.body; // Buffer when raw parsing is enabled
}
```
...
async handleText(@Req() req: UwsRequest) {
  ...
  console.log(req.body); // String when text parsing is enabled
}
```
...
handler(@Req() req: UwsRequest) {
  ...
  const tags = req.query.tags; // ['javascript', 'typescript', 'nodejs']
}
```
