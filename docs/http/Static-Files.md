...
downloadFile(@Param('filename') filename: string, @Res() res: UwsResponse) {
  res.attachment(filename);
  res.sendFile(`downloads/${filename}`);
}
```
...
@Get(':tenant/assets/*')
async serveTenantAssets(
  @Param('tenant') tenant: string,
  @Req() req: UwsRequest,
  @Res() res: UwsResponse,
) {
  const filepath = req.path.replace(`/${tenant}/assets/`, '');
  const fullPath = path.join('tenants', tenant, 'assets', filepath);
...
// Bad - no versioning
app.useStaticAssets('public', {
  maxAge: 31536000, // Cache issues on updates!
});
```
...
if (process.env.NODE_ENV === 'production') {
  ...
  app.use((req, res, next) => {
    res.setHeader('X-CDN-URL', 'https://cdn.example.com');
    next();
  });
} else {
